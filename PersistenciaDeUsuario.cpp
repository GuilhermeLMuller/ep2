...
Data *PersistenciaDeUsuario::lerData(ifstream &input)
{
    ...
    input >> month;
    input >> year;
    Data *d = new Data(hour, minute, second, day, month, year);
    return d;
}
...
vector<Usuario *> *PersistenciaDeUsuario::carregar(string arquivo)
{
    ...
    while (input)
    {
        ...
        if (tipo == 'F')
        {
            ...
            pessoa = new Funcionario(identificador, nome, registros);
            ...
            for (int j = 0; j < quantidadeDeRegistros; j++)
            {
                ...
                if (EntradaSaida == 'E')
                {
                    if (manual == 0)
                        pessoa->entrar(date);
                    if (manual == 1)
                        pessoa->registrarEntradaManual(date);
                }
                if (EntradaSaida == 'S')
                {
                    if (manual == 0)
                        pessoa->sair(date);
                    if (manual == 1)
                        pessoa->registrarSaidaManual(date);
                }
            }
        }
        ...
        if (tipo == 'V')
        {
            input >> identificador;
            input >> nome;
            dateInicio = lerData(input);
            dateFim = lerData(input);
...
void PersistenciaDeUsuario::salvar(string arquivo, vector<Usuario *> *v)
{
    ...
    for (int j = 0; j < v->size(); j++)
    {
        ...
        else if (dynamic_cast<Funcionario *>(v->at(j)))
        {
            ...
            for (int k = 0; k < func->getRegistros()->size(); k++)
            {
                ...
                if (dynamic_cast<Entrada *>(listaRegistros->at(k)))
                {
                    ...
                    output << " ";
                    if (listaRegistros->at(k)->isManual())
                        output << 1;
                    else
                        output << 0;
                }
        ...
        else if (dynamic_cast<Visitante *>(v->at(j)))
        {
